...
f^*(\hat{a}) \geq \sup_{a\in\mathcal{A}} f^*(a) - \epsilon.
  ...
  <p>We say that the learner is efficient if \(m(\epsilon, \delta)\), as well as its total running time, is \(\operatorname{poly}(n, 1/\epsilon, 1/\delta)\), where \(n\) is the representation length of the action set.</p>
</div>
